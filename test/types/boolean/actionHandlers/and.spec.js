...
const { actionHandlers, actionTypes: { ACTION_TYPE_AND } } = require('../../../../src/basic-types/boolean');

const actionReducer = actionHandlers[ACTION_TYPE_AND];
...
describe('when current state is TRUE', () => {
  ...
  it('applying TRUE should return TRUE', () => {
    const newState = actionReducer(oldState, { type: ACTION_TYPE_AND, payload: { value: true } });
    expect(newState).toBe(true);
  });

  it('applying FALSE should return FALSE', () => {
    const newState = actionReducer(oldState, { type: ACTION_TYPE_AND, payload: { value: false } });
