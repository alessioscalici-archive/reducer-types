...
const { actionHandlers, actionTypes: { ACTION_TYPE_DIVIDE } } = require('../../../../src/types/number');
...
describe('when current state is 0', () => {
  ...
  it('applying a positive number should return zero', () => {
    let newState = actionReducer(oldState, { type: ACTION_TYPE_DIVIDE, payload: { value: 42 } });
...
describe('when current state is a positive number', () => {
  ...
  it('applying a positive number should return the division (state / value)', () => {
    let newState = actionReducer(oldState, { type: ACTION_TYPE_DIVIDE, payload: { value: 5 } });
...
describe('when current state is NULL', () => {
  ...
  it('applying 0 should return NULL', () => {
    let newState = actionReducer(oldState, { type: ACTION_TYPE_DIVIDE, payload: { value: 0 } });
    expect(newState).toBe(null);
  });
});
